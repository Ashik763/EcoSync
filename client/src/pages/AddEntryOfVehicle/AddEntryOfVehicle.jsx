...
import { useGetRoleDetailsQuery, useGetStsManagerRoleDetailsQuery } from '../../Redux/features/auth/authApi';

const AddEntryOfVehicle = () => {
    const {id} =  useSelector(selectCurrentUser);
    console.log(id);

    const {data} =  useGetStsManagerRoleDetailsQuery(id);
    console.log(data);
    ...
    const handleSubmit = async (event) => {
       ...
       const volume_of_waste = form.weight_of_waste.value;
       ...
       const td = form.time_of_departure.value;
        ...
        const body = {
          sts_id: data?.result?.ward_number,
          vehicle_number:parseInt(vehicle_number),
          volume_of_waste:parseInt(volume_of_waste),
          time_of_departure: (time_of_departure),
          time_of_arrival: (time_of_arrival)   
        };
        console.log(body);
    
        fetch("http://localhost:5000/vehicle/entry", {
            method: "POST",
            headers: {
    ...
    return (
        ...
        <div className=" border login-content p-10 m-auto ">
          <form onSubmit={handleSubmit}>
            <div className="mb-3">
              <h2 className="text-center text-2xl"> Add Entry of Vehicle  </h2>
            </div>
...
export default AddEntryOfVehicle;